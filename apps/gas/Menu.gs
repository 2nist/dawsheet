  ...
  ui.createMenu("DAWSheet")
    ...
    .addSubMenu(
      ui
        ...
        .addItem("Timeline", "openTimelineSidebar")
    ...
    .addSubMenu(
      ui
        ...
        .addItem(
          ...
          "openArrangementTemplateSidebar"
        )
    )
    .addSeparator()
    .addSubMenu(
...
function openImportHubSidebar() {
  if (typeof import_openHubSidebar === "function")
    return import_openHubSidebar();
  SpreadsheetApp.getUi().alert(
    "Import Hub not wired yet. Use Import > Batch Import for now."
  );
}

// Bridge names from older code to new menu labels.
function openEditTriggersUi() {
  return openTriggerEditor();
}
function openRoutingSidebar() {
  return openRouting();
}
function util_libSelfTest() {
  try {
    ...
    SpreadsheetApp.getUi().alert(
      "Lib self-test failed: " + (e && e.message ? e.message : e)
    );
  }
}
function util_clearLibCache() {
  try {
    ...
    SpreadsheetApp.getUi().alert(
      "Clear cache failed: " + (e && e.message ? e.message : e)
    );
  }
}
function util_prewarmLibs() {
  try {
    ...
    SpreadsheetApp.getUi().alert(
      "Prewarm failed: " + (e && e.message ? e.message : e)
    );
  }
}
